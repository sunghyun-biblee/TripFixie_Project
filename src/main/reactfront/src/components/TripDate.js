...
const DateContainer = styled.div`
  font-weight: 600;
  input[type="date"] {
    background: url("/img/fairy.png") no-repeat right center/ 20px auto;
    border: none;
    position: relative;
...
function TripDate() {
  ...
  const [afterday, setAfterday] = useState(maxday());
  /*  
  ...
  */
  function maxday() {
    ...
    const setafterday = day.toISOString().split("T")[0];
    console.log(setafterday);
  ...
  const checkDate = (event) => {
    ...
    // const setafterday = maxday();
    // event.target.setAttribute("max", setafterday);
    setAfterday(maxday());
  };
  ...
  return (
    <DateContainer>
      <TripSelectDay>
        ...
        <div>
          <span>도착</span>
          <input type="date" id="tripend" data-placeholder="종료일" />
        </div>
      </TripSelectDay>
      <TripHour>
        <p>여행시간 설정</p>
